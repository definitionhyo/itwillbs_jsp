...
//			System.out.println("Idle : " + bds.getNumIdle()); // 유휴 상태 커넥션 수
	...
	//    리턴타입 : void
	// => 각각의 파라미터를 따로 전달받아 각각 close() 작업을 수행하도록 메서드를 정의
	//    이 때, 각각의 메서드 이름은 모두 close() 로 통일하고 파라미터만 다르게 지정
	...
	public static void commit(Connection con) {
		try {
			// Connection 객체의 commit() 메서드를 호출하여 Commit 작업 수행
			con.commit();
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public static void rollback(Connection con) {
		try {
			// Connection 객체의 rollback() 메서드를 호출하여 Rollback 작업 수행
			con.rollback();
		} catch (SQLException e) {
			e.printStackTrace();
		}
