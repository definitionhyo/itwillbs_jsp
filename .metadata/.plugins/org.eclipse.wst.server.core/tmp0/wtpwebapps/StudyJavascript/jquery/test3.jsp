...
// 		alert("ready 이벤트222");
		...
		// => 모든 요소에 대해 글자색("COLOR" 속성)에 대한 속성값을 "RED" 로 변경
		$("*").css("color", "red");
		
		// id 선택자(#) 중 "idSelector" 라는 이름을 갖는 요소의 글자색을 "BLUE" 로 변경
		$("#idSelector").css("color", "blue");
		// => "idSelector" 선택자의 "COLOR" 속성에 대한 속성값 접근(확인)
// 		alert($("#idSelector").css("color")); // rgb(0, 0, 255) = blue
		
		// class 선택자(.) 중 "classSelector" 라는 이름을 갖는 요소들의 글자색 "ORANGE" 로 변경
		$(".classSelector").css("color", "orange");
// 		alert($(".classSelector").css("color")); // rgb(255, 165, 0) = orange
		// ---------------------------------------------------------------------------
		// 태그 지정하기
		// table 태그 지정하여 대상의 테두리, 배경색 변경
		// => 별도의 선택자 기호 없이 태그명으로 접근
...
// 		$("table").css("background", "yellow");
		...
		// 동일한 대상에 복수개의 함수를 호출하여 속성을 변경할 경우
		// 각각의 함수를 연결하여 연쇄적으로 호출 가능하다!
// 		$("table").css("border", "1px solid blue").css("background", "pink");
		
		// 동일한 대상에 동일한 함수를 호출하는 경우
		// css() 함수 내에서 블록문{} 을 지정하여 복수개의 속성을 한번에 변경 가능
		// => 실제 CSS 와 유사함
		$("table").css({
...
// 		$("table tr").css("background", "green"); // 자식 또는 후손선택자
		
		// tr 태그들 중 첫번째 tr 태그의 배경색만 "green" 으로 변경
// 		$("table tr:first").css("background", "green");
		$("tr:first").css("background", "skyblue"); // table 태그 생략 가능
...
// 		alert("아이디 : " + id + ", 패스워드 : " + passwd); // 아이디 : admin, 패스워드 : 1234
	
		// id 선택자 "inputBox" 의 자식 중 
		// textarea 태그 속성이 readonly 인 요소의 배경색 lightgray 로 변경
		$("#inputBox textarea[readonly]").css("background", "lightgray");
		// --------------------------------------------------------
		// id 선택자 "divBox" 요소의 후손(공백)들인 div 태그 글자색 blue 로 변경
// 		$("#divBox div").css("color", "blue"); // 1, 1-1, 1-2, 2, 3, 3-1, 3-2 변경됨 
		
		// id 선택자 "divBox" 요소의 자식(>)들인 div 태그 배경색 pink 로 변경
		$("#divBox > div").css("color", "blue"); // 1, 2, 3 변경됨
		// --------------------------------------------------------
		// id 선택자 "selectBox" 요소에서 선택된 값(option 태그의 selected 속성이 있는 값)을
		// 가져와서 id 선택자 "selectResult" 인 요소에 표시(출력)하기
		// => 가상 선택자 활용 가능(option:selected)
// 		let selectedItem = $("#selectBox > option:selected").val(); // 선택값 가져오기

		// option 태그와 selected 가상 선택자만 사용해도 동일함
		let selectedItem = $("option:selected").val(); // 선택값 가져오기
		// val() 함수를 호출하여 파라미터로 값 지정 시 해당 태그의 value 속성값 변경
