...
// 	response.sendRedirect("attributeScopeTest2.jsp");
	...
	*/
	...
	하나의 요청/응답으로 처리되어 기존 request 객체가 그대로 유지됨
	따라서, 디스패치 방식으로 포워딩 된 페이지에서 request 객체의 속성에 접근 시 데이터 존재함  
	또한, 새로운 요청이 아니므로 웹브라우저 주소 표시줄의 주소(URL)가 그대로 유지됨
	=> pageContext 객체의 속성값은 현재 페이지를 벗어났으므로 null 값 출력됨
	=> request 객체의 속성값은 이전 요청이 그대로 유지됨(null 아님)
	=> session, application 객체의 속성값은 유지됨(null 아님)
	*/
	%>
</body>
