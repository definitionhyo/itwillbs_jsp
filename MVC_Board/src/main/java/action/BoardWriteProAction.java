...
public class BoardWriteProAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// ----------------------------------------------------------
			// 게시물 작성자(= 클라이언트)의 IP 주소를 알아내야하는 경우
...
//			MultipartRequest multi = new MultipartRequest(request, uploadPath, fileSize, realPath, null);
...
//			System.out.println(board.getBoard_name()); // null 출력됨
...
//			board.setBoard_file(multi.getParameter("board_file")); // null
			...
			//    실제 업로드 되는 파일명 : getFilesystemName()
...
//			System.out.println(board);
			// -------------------------------------------------------------
			// BoardWriteProService - registBoard() 메서드 호출
